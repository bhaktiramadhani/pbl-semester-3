        ...
        <section id="products">
            ...
            <div class="pb-4 bg-white dark:bg-gray-900">
                ...
                <div class="relative mt-1 flex flex-wrap justify-between gap-4">
                    ...
                    <button data-modal-target="add-modal" data-modal-toggle="add-modal" class="text-white bg-brown hover:bg-brownHover font-medium rounded-lg text-sm w-auto sm:w-auto px-5 py-2.5 text-center transition-all duration-200 ease-in-out <?php if (count($data['best_seller']) === 3) echo 'cursor-not-allowed'; ?>" <?php if (count($data['best_seller']) === 3) echo 'disabled title="sudah ada 3" '; ?>>Tambah Best Seller</button>
                </div>
            </div>
                        ...
                        <?php foreach ($data['best_seller'] as $bestSeller) : ?>
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4 space-y-2">
                                    <a href="#" data-modal-target="edit-modal<?= $bestSeller['id_products']; ?>" data-modal-toggle="edit-modal<?= $bestSeller['id_products']; ?>" class="font-medium text-white bg-brown hover:bg-brownHover py-2 px-4 rounded-lg inline-block">Edit</a>
                                    <a href="<?= BASEURL; ?>/dashboard/hapus_best_seller/<?= $bestSeller['id_best_seller'] ?>" class="font-medium text-white bg-red-600 hover:bg-red-500 py-2 px-4 rounded-lg inline-block">Hapus</a>
                                </td>
                            </tr>
                            <div id="edit-modal<?= $bestSeller['id_products']; ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative p-4 w-full max-w-2xl max-h-full">
                                    ...
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                            ...
                                            <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="edit-modal<?= $bestSeller['id_products']; ?>">
                                        ...
                                        <div class="p-4 md:p-5 space-y-4">
                                            <form action="handler/edit_data.php" method="post" enctype="multipart/form-data">
                                                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                                                    <input type="hidden" name="id" value="<?= $bestSeller['id_products']; ?>">
                                                    <div class="sm:col-span-2">
                                                        ...
                                                        <select id="category" name="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                            ...
                                                            <?php foreach ($products as $product) : ?>
                                                                <option <?php if ($bestSeller['id_products'] === $product['id_products']) echo 'selected' ?> value="<?= $product['name'] ?>"><?= $product['name'] ?></option>
                                                            <?php endforeach ?>
                                                        </select>
                                                    </div>
                                                    <div class="sm:col-span-2">
                                                        <label for="urutan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Urutan Produk</label>
                                                        <select id="urutan" name="urutan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                            <option disabled>Pilih Urutan</option>
                                                            <?php foreach ($urutans as $urutan) : ?>
                                                                <option <?php if ($urutan === (int)$bestSeller['urutan']) echo 'selected' ?> value="<?= $urutan ?>"><?= $urutan ?></option>
                                                            <?php endforeach; ?>
                                                        </select>
                                                    </div>
                                                </div>
    ...
    <div id="add-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <div class="p-4 md:p-5 space-y-4">
                    <form action="<?= BASEURL; ?>/dashboard/tambah_best_seller" method="post" enctype="multipart/form-data">
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div class="sm:col-span-2">
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Produk</label>
                        ...
                        <button type="submit" name="submit" class="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white bg-brown rounded-lg hover:bg-brownHover transition-all duration-200 ease-in-out">Tambah Best Seller</button>
                    </form>
                </div>
            </div>
        </div>
